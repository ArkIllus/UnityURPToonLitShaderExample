...
half3 GetFinalEmissionColor(Varyings input, float4 baseColorFinal)
{
    ...
    if(_UseEmission)
    {
        ...
        {   
            ...
            //TODO：这里不用baseColorFinal，用tex2D(_BaseMap, input.uv)，
            //但这样和上面的GetFinalBaseColor就采样了2次_BaseMap，重复了
...
// | 这个函数不包含光照逻辑，它只是传递光照结果数据，这个函数中完成的工作是“阴影映射 深度测试 世界位置偏移”？
half3 ShadeAllLights(ToonSurfaceData surfaceData, ToonLightingData lightingData)
{
    ...
    */
    ...
    half3 emissionRimLightResult = ShadeEmissionAndRim(surfaceData, lightingData, mainLight); //改进
    
    //==============================================================================================
    // 合成：间接光照 + 主光照 + 所有额外光照 + 自发光
